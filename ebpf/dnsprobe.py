...
parser.add_argument("-d", "--db", default="/tmp/dns.db",
                    help="on-disk cache of DNS results")
...
print(f"Opening local DNS cache at {args.db}")
...
def print_ipv4_event(cpu, data, size):
    ...
    src_ip = inet_ntop(AF_INET, pack("I", event.saddr)).encode()
    dest_ip = inet_ntop(AF_INET, pack("I", event.daddr)).encode()
    dnsval = print_dns(dest_ip)
    params = (src_ip, dest_ip, event.dport, dnsval)
    conn.execute("INSERT INTO dns VALUES (?, ?, ?, ?, datetime())", params) 
    conn.commit()
    if args.lport:
